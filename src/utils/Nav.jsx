...
const Nav = () => {
    ...
    return (
        <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 100 }} transition={{ duration: 0.7 }} className='w-full h-screen px-10 py-7 text-white absolute z-10 top-0 left-0'>
            <div className="max-w-[1700px] w-full h-auto flex items-center justify-between mx-auto">
                ...
                <ul className='items-center gap-x-12 flex lg:relative fixed right-0 lg:w-auto lg:h-auto w-full h-full top-0 justify-center lg:justify-start flex-col lg:flex-row bg-[#111111] lg:bg-transparent gap-y-20 text-xl lg:text-sm opacity-0 transition-all ease-in-out duration-300 lg:opacity-100' style={active ? { opacity: "100%" } : {}}>
                    ...
                    {list.map((items) => {
                        return (
                            <div className='flex flex-col items-center gap-y-[5px] select-none' key={items} >
                                <li className="uppercase font-medium cursor-pointer peer hover:text-header transition-colors ease-in-out duration-300">{items}</li>
